...
from bearvision.ports import ExtractedClip
    ...
    async def extract(
        ...
        *,
        ...
        destination.write_bytes(
            f"simulated-mp4:start={start_s}:duration={duration_s}".encode()
        )
...
def test_simulated_gopro_models_preview_hindsight_media_and_persistence(
    ...
    downloaded = controller.download_file(
        "100GOPRO/GX010001.MP4", str(tmp_path / "downloaded.mp4")
    )
...
def test_production_adapter_runs_against_disk_backed_gopro_simulator(
    ...
    async def exercise() -> None:
        ...
        assert first.local_path.read_bytes().startswith(b"simulated-mp4")
        assert first.asset.filename == "capture-1-GX010001.MP4"
