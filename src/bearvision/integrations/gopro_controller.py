...
from .gopro_config import GoProConfiguration, save_config_to_yaml, load_config_from_yaml
            ...
            try:
                ...
                if any(
                    ...
                    raise ConnectionError(
                        f"GoPro connection error during configuration upload: {e}"
                    )
                else:
        ...
        try:
            ...
            if config.video_resolution:
                ...
                resolution_value = (
                    ...
                    if hasattr(config.video_resolution, "value")
                    else config.video_resolution
                )
                if resolution_value == "4K":
                    ...
                    await self._gopro.http_setting.video_resolution.set(
                        settings.VideoResolution.NUM_2_7K
                    )
                elif resolution_value == "1080p":
                    await self._gopro.http_setting.video_resolution.set(
                        settings.VideoResolution.NUM_1080
                    )
            ...
            if config.frame_rate:
                ...
                frame_rate_value = (
                    ...
                    if hasattr(config.frame_rate, "value")
                ...
                if frame_rate_value == "60":
                    ...
                    await self._gopro.http_setting.frames_per_second.set(
                        settings.FramesPerSecond.NUM_30_0
                    )
                elif frame_rate_value == "24":
                    await self._gopro.http_setting.frames_per_second.set(
                        settings.FramesPerSecond.NUM_24_0
                    )
            ...
            if config.hindsight:
                # Handle both enum objects and string values
                hindsight_value = (
                    config.hindsight.value
                    if hasattr(config.hindsight, "value")
                    else config.hindsight
                )
