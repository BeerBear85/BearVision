...
class FullClipExtractor:
    ...
    def extract_single_clip(arg_option_obj: ConfigurationHandler, arg_clip_spec: BasicClipSpecification):
        ...
        tmp_duration = datetime.timedelta(seconds=float(arg_option_obj['FULL_CLIP_SPECIFICATION']['clip_duration']))
        tmp_output_video_relative_speed = float(arg_option_obj['FULL_CLIP_SPECIFICATION']['output_video_speed'])
        tmp_output_video_scale = float(arg_option_obj['FULL_CLIP_SPECIFICATION']['output_scale'])

        start_frame = int(tmp_input_video.fps * (arg_clip_spec.start_time - tmp_input_video.creation_time).total_seconds())
        output_fps = int(tmp_input_video.fps * tmp_output_video_relative_speed)
        clip_frame_duration = int(tmp_input_video.fps * tmp_duration.total_seconds())
        clip_frame_width = int(tmp_input_video.width * tmp_output_video_scale)
        clip_frame_height = int(tmp_input_video.height * tmp_output_video_scale)
