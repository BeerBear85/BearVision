...
class GoProConfiguration(BaseModel):
    ...
    video_resolution: Optional[VideoResolution] = Field(
        default=VideoResolution.RES_4K, description="Video recording resolution"
    )

    frame_rate: Optional[FrameRate] = Field(
        default=FrameRate.FPS_60, description="Video recording frame rate"
    )

    video_lens: Optional[VideoLens] = Field(
        default=VideoLens.WIDE, description="Video lens/field of view setting"
    )

    hindsight: Optional[HindsightOption] = Field(
        default=HindsightOption.SEC_15, description="HindSight buffer duration"
    )

    # Image stabilization
    hypersmooth: Optional[HyperSmooth] = Field(
        default=HyperSmooth.HIGH, description="HyperSmooth image stabilization level"
    )

    # Location and connectivity
    gps: Optional[GPS] = Field(default=GPS.ON, description="GPS tracking enable/disable")

    # Performance settings
    video_bit_rate: Optional[VideoBitRate] = Field(
        default=VideoBitRate.HIGH, description="Video encoding bit rate"
    )
    ...
    config_name: Optional[str] = Field(
        default="Default Configuration", description="Human-readable name for this configuration"
    )
...
def load_config_from_yaml(file_path: str) -> GoProConfiguration:
    ...
    if not isinstance(config_data, dict):
        raise ValueError(
            f"Configuration file {file_path} must contain a dictionary/object, got {type(config_data).__name__}"
        )
