...
class CutExtractor:
    ...
    def extract_full_clip_specifications(self, arg_clip_specification_list: list):
        ...
        for clip_spec in arg_clip_specification_list:
            self.__extract_single_clip(clip_spec)
        return

    def __extract_single_clip(self, arg_clip_spec):
        logger.info("Creating output file: " + arg_clip_spec.output_video_path)
        self.input_video.init(arg_clip_spec.video_file.path)

        start_frame = int(
            self.input_video.fps * (arg_clip_spec.start_time - self.input_video.creation_time).total_seconds())
        output_fps = int(self.input_video.fps * arg_clip_spec.output_video_relative_speed)
        clip_frame_duration = int(self.input_video.fps * arg_clip_spec.duration.total_seconds())
        clip_frame_width = int(self.input_video.width * arg_clip_spec.output_video_scale)
